#include "FlyMovementMode.h"

FlyMovementMode::FlyMovementMode() :
	Strategy(),
	m_go(nullptr),
	m_mc(nullptr),
...
void FlyMovementMode::updateAccumulator(double deltaTime) {
...
void FlyMovementMode::stepPhysicalMovement() {
	GameObject* go = m_go;
	MovementComponent* mc = m_mc;
	AABBCollider collider;
	if (m_chunkManager->getCubeByCoords(go->m_position) == nullptr || m_chunkManager->getCubeByCoords(go->m_position)->getBlockId() == BlockId::NONE) {
		return;
	}
	...
	double lerpFactor = AIR_VELOCITY_LERP_FACTOR * mc->m_airDampeningInfluence;
	//mc->m_velocity.z = cubicInterpolation();
	mc->m_velocity.y = mc->m_velocity.y + ((mc->m_targetVelocity.y - mc->m_velocity.y) * lerpFactor);
			...
			if (colliderVertexPosition.y < nearestCubes) {
				...
				while (vertical) {
					...
					while (horizontal) {
						...
						Block* cube = m_chunkManager->getCubeByCoords(colliderVertexPosition);
						//BOOST_LOG_TRIVIAL(info) << "CUBE1: " << m_chunkManager->getCubeAbsCoords(cube).y;
						cubeId = m_chunkManager->getCubeByCoords(colliderVertexPosition)->getBlockId();
						if (cubeId != BlockId::AIR) {
...
void FlyMovementMode::computeInterpolatedPosition(double alpha, GameObject* gameObject, MovementComponent* movementComponent) {
