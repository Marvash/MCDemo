...
Chunk::Chunk(GameServiceLocator* gameServiceLocator, int chunkHeight, int chunkSideSize, glm::vec3 chunkPosition) :
	...
	m_atlas = m_gameServiceLocator->getAtlas();
	m_biomeManager = m_gameServiceLocator->getBiomeManager();
	initEmptyBlockMatrix();
	renderingSetup();
}

Chunk::Chunk(GameServiceLocator* gameServiceLocator, Cube*** blockMatrix, int chunkHeight, int chunkSideSize, glm::vec3 chunkPosition) :
	leftNeighbour(nullptr),
	rightNeighbour(nullptr),
	frontNeighbour(nullptr),
	backNeighbour(nullptr),
	...
	for (int i = 0; i < chunkHeight; i++) {
		...
		for (int j = 0; j < chunkSideSize; j++) {
			...
			for (int k = 0; k < chunkSideSize; k++) {
				m_blockMatrix[i][j][k] = Cube(Cube::CubeId::AIR_BLOCK, this, nullptr);
			}
		}
	}
	...
	for (int i = 0; i < chunkHeight; i++) {
		for (int j = 0; j < chunkSideSize; j++) {
			for (int w = 0; w < chunkSideSize; w++) {
				if (m_blockMatrix[i][j][w].getCubeId() == Cube::CubeId::AIR_BLOCK) {
					continue;
				}
